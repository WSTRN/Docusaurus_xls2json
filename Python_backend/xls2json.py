...
def func_xls2json(xls_file):
    ...
    # Read the excel file
    ModeTable = pd.read_excel(xls_file,sheet_name='s_ModeTable',header=1,keep_default_na=False,usecols="A:M")
    ChlGroupTable = pd.read_excel(xls_file,sheet_name='s_ChlGroupTable',keep_default_na=False)
    PulseWidthTable = pd.read_excel(xls_file,sheet_name='s_PulseWidthTable',keep_default_na=False,usecols="A:F")
    DZCarrierTable = pd.read_excel(xls_file,sheet_name='s_DZCarrierTable',keep_default_na=False,usecols="A:C")
    OutputDataTable = pd.read_excel(xls_file,sheet_name='s_OutputDataTable',keep_default_na=False,usecols="A:C")
    options = pd.read_excel(xls_file,sheet_name='options',keep_default_na=False)
    ...
    # Write RecipeFile
    RecipeFile = []
    ParseType = '{:>02}'.format(options.loc[0, 'ParseType'])
    RecipeFile.append(ParseType)
    
    ModeCode = re.findall(".{2}", '{:>08}'.format(options.loc[0, 'ModeCode']))
    ModeCode.reverse()
    ModeCode = ' '.join(ModeCode)
    RecipeFile.append(ModeCode)
    
    ModeVersion = re.findall(".{2}", '{:>08}'.format(options.loc[0, 'ModeVersion']))
    ModeVersion.reverse()
    ...
    RecipeID = re.findall(".{2}", '{:>08}'.format(str(format(options.loc[0, 'RecipeID'], 'x'))))
    RecipeID = ' '.join(RecipeID)
    CfgChecksummedData.append(RecipeID)
    RecipeVersion = re.findall(".{2}", '{:>016}'.format(str(format(options.loc[0, 'RecipeVersion'], 'x'))))
    ...
    with open("./OutputRecipe.json",'w') as f:
...
class Xls2Json:
    ...
    def set_file(self, xls_file, json_file='default.json'):
        self.xls_file = xls_file
        self.json_file = json_file

    def convert(self):
        self.result = func_xls2json(self.xls_file)
...
if __name__ == '__main__':
    ...
    xls2json = Xls2Json()
    xls2json.set_file(xls_file, json_file)
    xls2json.convert()
